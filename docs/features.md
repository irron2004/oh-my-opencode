...
| `/ulr` | Document-first Ultra-Research workflow |
...
**Purpose**: Start document-first research convergence for a focused topic.
...
**Outputs**: creates/updates these artifacts under `.sisyphus/ulr/<topic>/`:
...
**Explicit note**: `/ulr` currently covers Phase 1+2 only. Phase 3 state-machine automation is not included yet.

**Troubleshooting**:
- `disabled_commands`: ensure `ulr` is not disabled.
- `disabled_hooks`: ensure `auto-slash-command` is not disabled if you expect slash expansion behavior.
